...
public class PeriodBankAccount extends Model {
	 ...
	 * @return
	 */
	public static PeriodBankAccount findByCashdeskNumberAndDate(
			int cashdeskNumber) {
		return new Select()
				.from(PeriodBankAccount.class)
				.where("BankAccountId = ? AND PeriodDate = ?",
						cashdeskNumber,
						(new JodaDateTimeSerializer()).serialize(DateTime.now()
								.withTime(0, 0, 0, 0))).executeSingle();
	}

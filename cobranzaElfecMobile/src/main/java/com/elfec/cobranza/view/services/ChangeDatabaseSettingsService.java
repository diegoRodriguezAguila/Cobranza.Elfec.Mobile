...
/**
 * Provee de un servicio de dialogo para cambiar las configuraciones de la base
 * de datos
 * 
 * @author drodriguez
 *
...
public class ChangeDatabaseSettingsService implements
	...
	public ChangeDatabaseSettingsService(Context context) {
		...
		croutonStyle = new de.keyboardsurfer.android.widget.crouton.Style.Builder()
				...
				.setBackgroundColorValue(
						ContextCompat.getColor(context, R.color.cobranza_color))
				.build();
		dialogView = ((LayoutInflater) context
				.getSystemService(Context.LAYOUT_INFLATER_SERVICE)).inflate(
				R.layout.dialog_change_database_settings, null);
		...
		txtRolePassword = (TextView) dialogView
				.findViewById(R.id.txt_role_password);
	 ...
	 * @param errors
	 */
	private void setFieldErrors(final TextView txtField,
			final List<String> errors) {
		mHandler.post(new Runnable() {
			@Override
			public void run() {
				if (errors.size() > 0)
					txtField.setError(MessageListFormatter
							.fotmatHTMLFromStringList(errors));
				else
					txtField.setError(null);
			}
		});
	}
	...
	public void notifyErrorsInFields() {
		...
		Crouton.makeText((Activity) context, R.string.errors_in_fields,
		...
		mHandler.post(new Runnable() {
			...
			public void run() {
				...
				Toast.makeText(context,
						R.string.msg_settings_saved_successully,
